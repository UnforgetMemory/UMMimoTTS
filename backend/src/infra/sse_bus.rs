...
pub fn spawn_sse_bridge(
    ...
    tokio::spawn(async move {
        tracing::info!("sse-bridge: started, waiting for events");
        loop {
            match event_rx.recv().await {
                Ok(event) => {
                    // Derive channel(s) from the event and publish
                    let channels = event_channels(&event);
                    tracing::info!("sse-bridge: received event, publishing to channels: {:?}", channels);
                    for ch in channels {
...
fn event_channels(event: &DomainEvent) -> Vec<String> {
    match event {
        ...
        | DomainEvent::TaskStatusChanged { task_id, batch_id: Some(bid), .. } => {
        ...
        | DomainEvent::TaskStatusChanged { task_id, batch_id: None, .. } => {
            vec![format!("task:{task_id}")]
        }
