...
use um_mimo_tts_server_v3::infra::persistence::db::create_pool;
...
async fn main() -> std::io::Result<()> {
    ...
    // ── rate limiter ──────────────────────────────────────────────────
    let rate_limiter = Arc::new(TokenBucket::new(60));
    ...
    // ── services ──────────────────────────────────────────────────────
    let task_service = Arc::new(TaskService::new(task_repo.clone(), task_queue.clone()));
